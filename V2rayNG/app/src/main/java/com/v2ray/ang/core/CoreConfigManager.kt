...
object CoreConfigManager {
    ...
    fun getV2rayConfig(context: Context, guid: String): ConfigResult = try {
    ...
    fun getV2rayConfig4Speedtest(context: Context, guid: String): ConfigResult = try {
        val cc = CoreConfigContextBuilder.build(context, guid) ?: return ConfigResult(false, guid, errorMessage = "Failed to build config context")
        ...
        ConfigResult(false, guid, errorMessage = "Failed to get V2ray config for speedtest: ${e.message ?: e.javaClass.simpleName}")
    }
    ...
    private fun buildV2rayCustomConfig(cc: CoreConfigContext): ConfigResult {
        ...
        if (inbounds.none { it.isJsonObject && it.asJsonObject.get("protocol")?.asString == "tun" }) {
            initV2rayConfig(cc).inbounds.firstOrNull { it.tag == "tun" }?.let { tun ->
                tun.settings?.mtu = SettingsManager.getVpnMtu()
                inbounds.add(JsonUtil.parseString(JsonUtil.toJson(tun)))
            }
        }
    ...
    private fun buildUnifiedConfig(cc: CoreConfigContext): V2rayConfig {
        ...
        val strategies = mutableListOf<BalancerStrategy>()
        cc.resolvedOutbounds.forEachIndexed { i, spec -> buildOutbounds(i == 0, spec, tags, config, groups, strategies) }
        configureRouting(cc, config, groups)
        configureFakeDns(config)
        configureDns(cc, config, groups)
        configureLocalDns(cc, config)
        configureRootModeDns(config)
        ...
        applyObservability(config, strategies)
        applySpeedDisabled(config)
        resolveOutboundDomainsToHosts(config)
        return config
    }
    ...
    private fun handleChain(spec: CoreConfigContext.ResolvedOutbound, prepend: Boolean, tags: MutableSet<String>, config: V2rayConfig) {
        ...
        if (list.size == 1) {
            list[0].tag = spec.tag
            if (prepend) config.outbounds.add(0, list[0]) else config.outbounds.add(list[0])
            tags.add(spec.tag)
            return
        }
        ...
        if (prepend) config.outbounds.addAll(0, list) else config.outbounds.addAll(list)
        list.forEach { tags.add(it.tag) }
    }
    ...
    private fun handleGroup(spec: CoreConfigContext.ResolvedOutbound, prepend: Boolean, tags: MutableSet<String>, config: V2rayConfig, groups: MutableMap<String, String>, strategies: MutableList<BalancerStrategy>) {
        ...
        val prefix = "${AppConfig.TAG_PROXY}-${spec.tag}-"
        val members = mutableListOf<V2rayConfig.OutboundBean>()
        pairs.forEachIndexed { i, pair ->
            val tag = "$prefix${i + 1}-${pair.second.remarks.trim()}"
            if (tag !in tags) {
                pair.first.tag = tag
                members.add(pair.first)
                tags.add(tag)
            }
        }
        ...
        val type = BalancerStrategyType.from(spec.profile.policyGroupType)
        val fallback = if (type.supportsObservatory && spec.profile.policyGroupTestOutbounds != false) spec.profile.policyGroupFallbackTag?.takeIf { it.isNotEmpty() && it != AppConfig.TAG_PROXY } ?: members.first().tag else null
        val strategy = buildBalancerStrategy(type, listOf(prefix), balancerTag, fallback)
        val bs = config.routing.balancers?.toMutableList() ?: mutableListOf()
        if (bs.none { it.tag == balancerTag }) {
            bs.add(strategy.balancer)
            config.routing.balancers = bs
        }
        strategies.add(strategy)
        groups[spec.tag] = balancerTag
    }

    private fun postProcessForSpeedtest(config: V2rayConfig) {
        config.log.loglevel = MmkvManager.decodeSettingsString(AppConfig.PREF_LOGLEVEL) ?: "warning"
        ...
        config.stats = null
        config.policy = null
        config.outbounds.forEach { it.mux = null }
    }
    ...
    private fun initV2rayConfig(cc: CoreConfigContext): V2rayConfig {
    ...
    private fun configureInbounds(config: V2rayConfig) {
        val inbound = config.inbounds[0]
        if (inbound.settings == null) inbound.settings = V2rayConfig.InboundBean.InSettingsBean()
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_PROXY_SHARING) != true) inbound.listen = AppConfig.LOOPBACK
        inbound.port = SettingsManager.getSocksPort()
        inbound.settings?.udp = MmkvManager.decodeSettingsBool(AppConfig.PREF_SOCKS_ENABLE_UDP, true)
        val u = SettingsManager.getSocksUsername()
        val p = SettingsManager.getSocksPassword()
        if (u != null && p != null) {
            inbound.settings?.auth = "password"
            inbound.settings?.accounts = listOf(V2rayConfig.InboundBean.InSettingsBean.SocksAccountBean(u, p))
        } else {
            inbound.settings?.auth = "noauth"
            inbound.settings?.accounts = null
        }
        val sniff = MmkvManager.decodeSettingsBool(AppConfig.PREF_SNIFFING_ENABLED, true) != false
        inbound.sniffing?.enabled = sniff
        inbound.sniffing?.routeOnly = MmkvManager.decodeSettingsBool(AppConfig.PREF_ROUTE_ONLY_ENABLED, false)
        if (!sniff) inbound.sniffing?.destOverride?.clear()
        if (!Utils.isXray()) {
            val h = JsonUtil.fromJson(JsonUtil.toJson(inbound), V2rayConfig.InboundBean::class.java) ?: return
            h.tag = EConfigType.HTTP.name.lowercase()
            h.port = SettingsManager.getHttpPort()
            h.protocol = EConfigType.HTTP.name.lowercase()
            config.inbounds.add(h)
        }
        if (needTun()) config.inbounds.firstOrNull { it.tag == "tun" }?.settings?.mtu = SettingsManager.getVpnMtu()
    }

    private fun configureFakeDns(config: V2rayConfig) {
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_LOCAL_DNS_ENABLED) == true && MmkvManager.decodeSettingsBool(AppConfig.PREF_FAKE_DNS_ENABLED) == true) config.fakedns = listOf(V2rayConfig.FakednsBean())
    }

    private fun configureDns(cc: CoreConfigContext, config: V2rayConfig, groups: Map<String, String>) {
        val servers = ArrayList<Any>()
        val remote = SettingsManager.getRemoteDnsServers()
        val domestic = SettingsManager.getDomesticDnsServers()
        remote.forEach { servers.add(it) }
        val hosts = buildDnsHostsFromRoutingRules(cc)
        val domesticTags = buildDnsFromRoutingRules(cc, servers, remote, domestic)
        config.dns = V2rayConfig.DnsBean(servers = servers, hosts = hosts, tag = AppConfig.TAG_DNS, enableParallelQuery = if ((domestic.size + remote.size) > 2) true else null)
        if (domesticTags.isNotEmpty()) config.routing.rules.add(V2rayConfig.RoutingBean.RulesBean(outboundTag = AppConfig.TAG_DIRECT, inboundTag = ArrayList(domesticTags)))
        val b = groups[AppConfig.TAG_PROXY]
        if (b != null) config.routing.rules.add(V2rayConfig.RoutingBean.RulesBean(balancerTag = b, inboundTag = arrayListOf(AppConfig.TAG_DNS))) else config.routing.rules.add(V2rayConfig.RoutingBean.RulesBean(outboundTag = AppConfig.TAG_PROXY, inboundTag = arrayListOf(AppConfig.TAG_DNS)))
    }

    private fun buildDnsHostsFromRoutingRules(cc: CoreConfigContext): MutableMap<String, Any> {
        val h = mutableMapOf<String, Any>()
        cc.routingDomainRules.filter { it.outboundTag == AppConfig.TAG_BLOCKED }.flatMap { it.domain }.forEach { h[it] = AppConfig.LOOPBACK }
        val user = MmkvManager.decodeSettingsString(AppConfig.PREF_DNS_HOSTS)
        if (user.isNotNullEmpty()) user?.split(",").orEmpty().filter { it.contains(":") }.forEach { val x = it.split(":", limit = 2); h[x[0].trim()] = x[1].trim() }
        return h
    }

    private fun buildDnsFromRoutingRules(cc: CoreConfigContext, servers: ArrayList<Any>, remote: List<String>, domestic: List<String>): MutableList<String> {
        val tags = mutableListOf<String>()
        cc.routingDomainRules.forEachIndexed { i, r ->
            when (r.outboundTag) {
                AppConfig.TAG_DIRECT -> domestic.forEachIndexed { j, a ->
                    val tag = "${AppConfig.TAG_DOMESTIC_DNS}_${i}_$j"
                    servers.add(V2rayConfig.DnsBean.ServersBean(address = a, domains = r.domain, skipFallback = true, tag = tag))
                    tags.add(tag)
                }
                AppConfig.TAG_BLOCKED -> Unit
                else -> if (remote.isNotEmpty()) servers.add(V2rayConfig.DnsBean.ServersBean(address = remote.first(), domains = r.domain))
            }
        }
        return tags
    }

    private fun configureLocalDns(cc: CoreConfigContext, config: V2rayConfig) {
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_LOCAL_DNS_ENABLED) != true) return
        if (SettingsManager.isVpnMode()) {
            val tag = if (SettingsManager.isUsingHevTun()) "socks" else "tun"
            config.routing.rules.add(0, V2rayConfig.RoutingBean.RulesBean(inboundTag = arrayListOf(tag), outboundTag = "dns-out", port = "53"))
        }
        if (config.outbounds.none { it.protocol == "dns" && it.tag == "dns-out" }) config.outbounds.add(V2rayConfig.OutboundBean(protocol = "dns", tag = "dns-out", settings = null, streamSettings = null, mux = null))
    }

    private fun configureRootModeDns(config: V2rayConfig) {
        if (!SettingsManager.isRootMode()) return
        if (config.routing.rules.none { it.outboundTag == "dns-out" && it.port == "53" }) config.routing.rules.add(0, V2rayConfig.RoutingBean.RulesBean(inboundTag = arrayListOf("socks"), outboundTag = "dns-out", port = "53"))
        if (config.outbounds.none { it.protocol == "dns" && it.tag == "dns-out" }) config.outbounds.add(V2rayConfig.OutboundBean(protocol = "dns", tag = "dns-out", settings = null, streamSettings = null, mux = null))
    }

    private fun applySpeedDisabled(config: V2rayConfig) {
        if (MmkvManager.decodeSettingsBool(AppConfig.PREF_SPEED_ENABLED) != true) {
            config.stats = null
            config.policy = null
        }
    ...
    private fun resolveOutboundDomainsToHosts(config: V2rayConfig) {
        if (MmkvManager.decodeSettingsString(AppConfig.PREF_OUTBOUND_DOMAIN_RESOLVE_METHOD, "1") != "1") return
        val dns = config.dns ?: return
        val hosts = dns.hosts?.toMutableMap() ?: mutableMapOf()
        val ipv6 = MmkvManager.decodeSettingsBool(AppConfig.PREF_PREFER_IPV6) == true
        config.getAllProxyOutbound().forEach { ob ->
            val domain = ob.getServerAddress() ?: return@forEach
            if (domain == AppConfig.LOOPBACK) return@forEach
            val ips = HttpUtil.resolveHostToIP(domain, ipv6) ?: return@forEach
            ob.ensureSockopt().domainStrategy = "UseIP"
            ob.ensureSockopt().happyEyeballs = V2rayConfig.OutboundBean.StreamSettingsBean.HappyEyeballsBean(prioritizeIPv6 = ipv6, interleave = 2)
            hosts[domain] = if (ips.size == 1) ips[0] else ips
        }
        dns.hosts = hosts
    }
    ...
    private fun configureRouting(cc: CoreConfigContext, config: V2rayConfig, groups: Map<String, String>) {
        config.routing.domainStrategy = MmkvManager.decodeSettingsString(AppConfig.PREF_ROUTING_DOMAIN_STRATEGY) ?: "AsIs"
        MmkvManager.decodeRoutingRulesets()?.forEach { item ->
            if (item.enabled) {
                val rule = JsonUtil.fromJson(JsonUtil.toJson(item), V2rayConfig.RoutingBean.RulesBean::class.java) ?: return@forEach
                if (SettingsManager.canUseProcessRouting()) {
                    rule.process?.let { rule.process = PackageUidResolver.packageNamesToUids(cc.context, it).ifEmpty { null } }
                } else {
                    rule.process = null
                }
                groups[rule.outboundTag]?.let {
